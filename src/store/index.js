...
export default new Vuex.Store({
  ...
  mutations: {
    ...
    removeFromRoom(state, moveData) {
      let room = state.rooms.find(r => r.id == moveData.oldRoomId)
      room.items = room.items.filter(i => i.id != moveData.itemToMove.id)
    },
    addToRoom(state, moveData) {
      let room = state.rooms.find(r => r.id == moveData.newRoomId)
      room.items.push(moveData.itemToMove)
    }
  },
  actions: {
    setItemToMove({ commit, dispatch }, data) {
      commit("setItemToMove", data)
    },
    moveItem({ commit, dispatch }, moveData) {
      console.log(moveData);
      commit("removeFromRoom", moveData)
      commit("addToRoom", moveData)
    }
  },
  modules: {
  }
